...
namespace ModernCppCI {
  ...
  namespace Test {
    ...
    class SimpleObject {
    public:
      SimpleObject(const string& name, const unsigned int& age) {
        name_ = name;
        age_ = age;
      }
    ...
    TEST_CASE("logger work as expected", "[logger]") {
      ...
      SECTION("the section is correct") {
        REQUIRE(log.section() == "logger test");
      }
      ...
      SECTION("we could logging with level as arg") {
        log.log(info, "this is simple logging with {} as arg", "level");
        REQUIRE(true);
      }
